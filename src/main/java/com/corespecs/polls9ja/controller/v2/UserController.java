...
import com.corespecs.polls9ja.exception.BindingErrorsResponse;
...
public class UserController {
    ...
    @ApiOperation(value = "Creates a new User", notes="The newly created user Id will be sent in the location response header",
            response = Void.class)
    @ApiResponses(value = {@ApiResponse(code=201, message="User Created Successfully", response=Void.class),
            @ApiResponse(code=500, message="Error creating User", response= ErrorDetail.class) } )
    public ResponseEntity<Void> addOwner(@RequestBody @Valid User user) throws Exception {
